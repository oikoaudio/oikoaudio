...
use crate::{nice_trace, plugin::Plugin};
    ...
    pub fn from_midi(timing: u32, midi_data: &[u8]) -> Result<Self, u8> {
        ...
        match S::from_buffer(midi_data) {
            ...
            None => {
                if event_type == 0xf0 {
                    if midi_data.len() <= 32 {
                        nice_trace!("Unhandled MIDI system message: {midi_data:02x?}");
                    } else {
                        nice_trace!("Unhandled MIDI system message of {} bytes", midi_data.len());
                    }
                } else {
                    nice_trace!("Unhandled MIDI status byte {status_byte:#x}");
                }
