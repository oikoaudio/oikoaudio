...
struct mtsclientglobal
{
    ...
    void load_lib()
    {
        // An explicit override also isolates tests from a running DAW's master.
        const char *override_path = std::getenv("WEFT_MTS_LIBRARY");
        if (override_path) {
            handle = dlopen(override_path, RTLD_NOW);
            if (!handle) return;
        } else if (!(handle = dlopen("/Library/Application Support/MTS-ESP/libMTS.dylib", RTLD_NOW)) &&
